...
class ThreeLevelSUUExclusiveBypassingCacheSystem:
            ...
            if block is None:
                ...
                if block is None:
                    ...
                    if evicted:
                        ...
                        replacement = Block(evicted.base_address() & self.UL3.get_base_address_mask(), evicted.is_dirty(), self.UL3.get_policy())
                        # Shouldn't be a eviction
                        self.UL3.put(replacement)
            else:
                block.read()
                cache.remove(block)
                ...
                if evicted:
                    ...
                    replacement = Block(evicted.base_address() & self.UL2.get_base_address_mask(), evicted.is_dirty(), self.UL2.get_policy())
                    # Shouldn't be a eviction
                    self.UL2.put(replacement)
        else:
            block.read()
